...
class Solution {
 public:
  ...
  // Saves mapping of original node to cloned node.
  Node *cloneGraph(Node *node) {
    if (node == nullptr)
      return nullptr;
    if (hash.find(node) == hash.end()) { // Never seen this orig node before.
      hash[node] = new Node(node->val, {});
      // Make a copy corresponding to orig node at hash[node].
...
void printGraph(Node *node) {
  unordered_map<Node *, bool>
    hash; // Second parameter defines whether this has been printed or not.
  hash[node] = false;
  for (unordered_map<Node *, bool>::iterator it = hash.begin();
       it != hash.end();) {
    if (it->second == false) {
