...
class Solution {
  ...
  /**
     ...
     */
  void wordSquares(trie *root, int len, vector<string> &square,
                   vector<vector<string>> &result) {
    if (square.size() ==
        len) { // If I have enough words in my square. Put it in result.
 ...
 public:
  ...
  vector<vector<string>> wordSquares(vector<string> &words) {
    ...
    for (auto word : words) {
      ...
      square.push_back(
        word); // Lets try building square with this word. It were possible, we would put it in
