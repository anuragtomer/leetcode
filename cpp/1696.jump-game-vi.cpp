    ...
    for (int i = nums.size() - 1; i >= 0; --i) {
      curr =
        ...
        (dq.empty()
           ? 0
           : nums
               [dq.front()]); // Get current best, the maximum result will be the
                              // first element in the monoqueue.
      ...
      if (dq.front() >= i + k)
        dq.pop_front();
      nums[i] =
        curr; // Use input array as auxillary array to store the best results.
    }
