 ...
 public:
  ...
  pair<float, int> dfs(TreeNode *node, bool fixed) {
    ...
    if (fixed == false) {
      ...
      changes =
        leftFree.second + rightFixed.second + (node->val == avg ? 0 : 1);
      if (changes < minChanges) {
        minChanges = changes;
        value = avg;
      }
      ...
      changes =
        leftFixed.second + rightFree.second + (node->val == avg ? 0 : 1);
      if (changes < minChanges) {
        changes = minChanges;
        value = avg;
      }
      ...
      if (node->left) {
        ...
        auto rightFixed = dfs(node->right, true);
        changes =
          leftFixed.second + rightFixed.second + (leftVal == temp ? 0 : 1);
        if (changes < minChanges)
          minChanges = changes;
        node->left->val = temp;
      }
      ...
      if (node->right) {
        ...
        auto rightFixed = dfs(node->right, true);
        changes =
          leftFixed.second + rightFixed.second + (rightVal == temp ? 0 : 1);
