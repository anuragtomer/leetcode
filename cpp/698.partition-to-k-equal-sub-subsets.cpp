...
class Solution {
 public:
  ...
  bool canPartitionKSubsets(vector<int> &nums, vector<int> subsetSum,
                            vector<bool> taken, int subset, int K, int N,
    ...
    for (int i = limitIdx; i >= 0; i--) {
      ...
      if (tmp <= subset) {
        ...
        bool nxt = canPartitionKSubsets(nums, subsetSum, taken, subset, K, N,
                                        curIdx, i - 1);
        taken[i] = false;
        subsetSum[curIdx] -= nums[i];
        if (nxt)
