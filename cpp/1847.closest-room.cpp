 ...
 public:
  vector<int> closestRoom(vector<vector<int>> &rooms, vector<vector<int>> &qs) {
    ...
    sort(begin(rooms), end(rooms),
         by_size_desc); // Sort rooms in decreasing order of their sizes.
    sort(
      begin(qs), end(qs),
      by_size_desc); // Sort queries in decreasing order of their preferred sizes.
    // The above sorting helps in building set incrementally.
    set<int> ids;
    for (int i = 0, j = 0; i < qs.size(); ++i) {
      auto prefRoomId = qs[i][0], minRoomSizeRequired = qs[i][1],
           queryNum = qs[i][2];
      ...
      int id1 =
        (it == begin(ids))
          ? -1
          : *(prev(it)); // This saves just smaller room, if there is any.
      int id2 =
        (it == end(ids))
          ? -1
          : *it; // This saves just greater or equal room, if there is any.
