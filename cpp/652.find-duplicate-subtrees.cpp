...
class Solution {
  string findDuplicateSubtree(
    TreeNode *root, unordered_map<string, TreeNode *> &preorderToTreeRoot,
    unordered_map<string, bool> &pushedInResult, vector<TreeNode *> &result) {
    if (!root) {
      return "";
    }
    string left = findDuplicateSubtree(root->left, preorderToTreeRoot,
                                       pushedInResult, result);
    string current = left + "," + to_string(root->val) + ",";
    string right = findDuplicateSubtree(root->right, preorderToTreeRoot,
                                        pushedInResult, result);
 ...
 public:
  vector<TreeNode *> findDuplicateSubtrees(TreeNode *root) {
    ...
    string temp =
      findDuplicateSubtree(root, preorderToTreeRoot, pushedInResult, result);
    return result;
  }
};
