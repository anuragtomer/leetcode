  ...
  void dfs(vector<vector<int>> &matrix, vector<vector<int>> &result) {
    ...
    for (int i = n - 2; i >= 0; --i) {
      for (int j = 0; j < n; ++j) {
        if (j == 0)
          result[i][j] =
            matrix[i][j] + min(result[i + 1][j], result[i + 1][j + 1]);
        else if (j == n - 1)
          result[i][j] =
            matrix[i][j] + min(result[i + 1][j - 1], result[i + 1][j]);
        else
          result[i][j] =
            matrix[i][j] +
            min({result[i + 1][j - 1], result[i + 1][j], result[i + 1][j + 1]});
      }
    }
  }
 ...
 public:
  int minFallingPathSum(vector<vector<int>> &matrix) {
    vector<vector<int>> result(matrix.size(),
                               vector<int>(matrix.size(), INT_MAX));
    if (matrix.size() == 1) {
      int sum = 0;
