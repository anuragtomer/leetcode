 ...
 public:
  ...
  bool remove(int val) {
    if (hash.find(val) != hash.end()) {
      ...
      hash[last] = hash
        [val]; // Put new index for last element as the index of the element we are going to remove.
      nums[hash[val]] =
        last; // Put last element at position of the element being removed.
      nums
        .pop_back(); // Either remove the last element, or reduce the size of array by 1.
