 ...
 public:
  vector<vector<string>> displayTable(vector<vector<string>> &orders) {
    ...
    map<int, map<string, int>>
      tableOrders; // Map from table to <dishes, quantity> ordered on that table.
                   // Again, sorted by table number.
    for (auto &order : orders) {
      // Order is <customer_name, table_number, dish_name>.
      foodDishes.insert(order[2]);
      tableOrders[stoi(order[1])]
                 [order[2]]++; // It, by defaults, starts from 0, I guess.
    }
    ...
    for (auto &order : tableOrders) {
      ...
      table.back().push_back(to_string(
        order.first)); // Add table number in last vector pushed in table.
      for (auto &foodDish : foodDishes)
        table.back().push_back(to_string(
          order.second[foodDish])); // Push back all the counts for each dish.
