...
class Solution {
  int prime =
    1e8 - 11; // For mod operations on current hash. Large enough prime number.
  vector<int>
    power; // Power is used to retrieve the index^26, so that we don't compute it again and again.
  string validate(int len, string str) {
    ...
    if (len == 0)
      return "";
    long long current =
      0; // This is the running hash of the substring of length 'len'.
    ...
    for (int i = len; i < str.size(); ++i) {
      ...
      current =
        ((current - (long long)power[len - 1] * (str[i - len] - 'a')) % prime +
         prime) %
        prime;
      ...
      if (hash.find(current) == hash.end())
        ...
        hash[current] =
          vector<int>(1, i - len + 1); // This substring starts at i - len + 1.
      else {
